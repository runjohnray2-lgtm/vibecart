...
test("uptime runner is secret-gated and returns a bounded execution summary", async () => {
  const route = await readFile("app/api/cron/uptime/route.ts", "utf8")

  assert.match(route, /process\.env\.CRON_SECRET/)
  assert.match(route, /authorization/)
  assert.match(route, /Bearer \$\{secret\}/)
  assert.match(route, /runDueUptimeChecks\(limit\)/)
  assert.match(route, /Math\.min\(Math\.max\(Math\.trunc\(rawLimit\), 1\), 100\)/)
  assert.match(route, /checked: results\.length/)
  assert.match(route, /down: results\.filter/)
})
